...
async def trigger_aggregation():
    ...
    async with model_lock:
        ...
        # 2. Aggregation Math
        try:
            aggregated_weights = trimmed_mean(updates_to_process, trim_ratio=0.2)
        except Exception as e:
        ...
        storage.log_aggregation(new_vid, accepted=accepted_clients_count, rejected=0, method="Trimmed Mean")
        storage.log_evaluation(new_vid, metrics['loss'], metrics['accuracy'])
        
        current_version_id = new_vid
        logger.info(f"Aggregation Round {current_version_num} Complete! Loss: {metrics['loss']:.4f}, Acc: {metrics['accuracy']:.4f}")
...
async def get_metrics():
    ...
    evals = storage.supabase.table("evaluation_metrics").select("*").order("version_id", desc=True).limit(20).execute()
    aggs = storage.supabase.table("aggregation_logs").select("*").order("version_id", desc=True).limit(20).execute()
...
async def receive_update(
    ...
    async with model_lock:
        ...
        logger.info(f"Accepted Client {client_id}. Queue size: {q_len}/{MIN_UPDATE_QUEUE}")
...
@app.post("/simulate")
async def run_local_simulation(background_tasks: BackgroundTasks, req: SimulationRequest):
    """
    ...
    def local_train_worker(name: str, malicious: bool, mult: float, vid: int):
        ...
        status, reason, norm_val, dist_val = detect_update(client_weights, asyncio.run(fetch_w()))
    ...
    background_tasks.add_task(local_train_worker, req.client_name, req.is_malicious, req.malicious_multiplier, current_version_id)

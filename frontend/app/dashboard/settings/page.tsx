...
import { Shield, SlidersHorizontal, RotateCcw, CheckCircle2, Save } from "lucide-react";
...
export default function SettingsPage() {
    ...
    const handleReset = () => {
        setConfig({ dp_enabled: true, dp_clip_norm: 10.0, dp_noise_mult: 1.0, min_update_queue: 3 });
    };
    ...
    return (
        <div className="flex flex-col gap-8 pb-10 max-w-3xl">
            <div className="flex flex-col gap-2">
                ...
                <p className="text-slate-500">Tune Differential Privacy parameters and federation thresholds. Changes take effect on the next aggregation round.</p>
            </div>
                ...
                <CardContent className="pt-6 flex flex-col gap-6">
                    ...
                    <div className="flex items-center justify-between py-2 border-b border-slate-100">
                        <div>
                            ...
                            <p className="text-sm text-slate-500">If disabled, pure Trimmed Mean is applied without noise (ablation mode).</p>
                        </div>
                        <button
                    ...
                    <div className={`flex flex-col gap-3 transition-opacity ${config.dp_enabled ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-slate-500">Max L2 norm for gradient clipping. Sensitivity bound.</p>
                            </div>
                            <span className="bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold font-mono text-sm px-3 py-1.5 rounded-lg min-w-[56px] text-center">{config.dp_clip_norm.toFixed(1)}</span>
                        </div>
                        <input
                    ...
                    <div className={`flex flex-col gap-3 transition-opacity ${config.dp_enabled ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-slate-500">σ = multiplier × C / n clients. Higher = stronger ε-DP guarantee.</p>
                            </div>
                            <span className="bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold font-mono text-sm px-3 py-1.5 rounded-lg min-w-[56px] text-center">{config.dp_noise_mult.toFixed(2)}</span>
                        </div>
                        <input
                ...
                <CardHeader className="border-b border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500">Control how many updates must queue before triggering an aggregation.</p>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="pt-6 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-slate-500">Minimum number of valid client updates before aggregation fires.</p>
                        </div>
                        <span className="bg-blue-50 border border-blue-200 text-blue-700 font-bold font-mono text-sm px-3 py-1.5 rounded-lg min-w-[48px] text-center">{config.min_update_queue}</span>
                    </div>
                    <input
